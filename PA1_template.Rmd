...
## interval.  The following commented code using dplyr's mutate function is my direction to
# tackle the problem but it just replace NAs data for the first day.
...
print(newQuantileX)
```
After imputing missing values, new values for the mean and the median of total number of steps taken per day are <strong>`r sprintf("%.2f", newMeanX)`</strong> and <strong>`r sprintf("%.2f", newQuantileX)`</strong>.  The mean remains the same, but the median moves closer to the mean (in this case, they are almost identical). In other words, the more data that is presented, the more concentrated density function is arounf the sample mean.
